...
Begin VB.UserControl jcbutton 
   ...
   MouseIcon       =   "jcButton.ctx":0000
   MousePointer    =   99  'Custom
   ScaleHeight     =   292
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   378
   ToolboxBitmap   =   "jcButton.ctx":0152
End
...
Public Enum enumButtonStlyes
    ...
    [eMyFrameButton]           '16) A Cool Frame with transparency
End Enum
...
Public Enum eBackStyle
    [eBackTransparency]        'Back Transparency of the JC Button
    [eBackOpaque]              'Back Opaque of the JC Button
End Enum
'-------------------------------------------------------------------
...
Private Enum GradientDirectionCts
    [gdHorizontal] = 0
    [gdVertical] = 1
...
Private mBorderStyle        As Boolean              'Mostra as bordas ou não
'Private mBackStyle         As Boolean              'Define a transparência do objecto
Private mBackStyle          As eBackStyle
...
'****************************************************************************
    ...
    Case eMyFrameButton
        DrawMyFrameButton m_Buttonstate
    End Select
...
Private Sub DrawMyFrameButton(ByVal vState As enumButtonStates)
...
Dim bColor As Long
    ...
    If mBackStyle = True Then
        Call pvMakeTransparent
            If mBorderStyle = True Then
                Select Case vState
                    Case eStateNormal
                        'PaintRect bColor, m_ButtonRect
                        'DrawGradientEx 0, 0, lw, lh / 2.7, BlendColors(ShiftColor(bColor, 0.09), TranslateColor(vbWhite)), BlendColors(ShiftColor(bColor, 0.07), bColor), gdVertical
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        mBorderStyle = .ReadProperty("BorderStyle", False)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        .WriteProperty "BackStyle", mBackStyle, False
        .WriteProperty "BorderStyle", mBorderStyle, False
...
Attribute About.VB_UserMemId = -552
    MsgBox "MyCommandButton" & vbNewLine & "A Multistyle Button Control" & vbNewLine & vbNewLine & "Created by: Juned S. Chhipa" & vbNewLine & "New Changes By: OrangeFlame", vbInformation + vbOKOnly, "About"
End Sub
...
Public Property Let BackStyle(ByVal New_Enabled As Boolean)
    mBackStyle = New_Enabled
    UserControl.Enabled = mBackStyle
    RedrawButton
...
Public Property Let BorderStyle(ByVal New_Enabled As Boolean)
    mBorderStyle = New_Enabled
    UserControl.Enabled = mBorderStyle
    RedrawButton
...
Private Sub pvMakeTransparent()
    ...
    With UserControl.Extender.Container
        .AutoRedraw = True
        BitBlt UserControl.hDC, 0, 0, .ScaleX(UserControl.Width, .ScaleMode, vbPixels), .ScaleY(UserControl.Height, .ScaleMode, vbPixels), .hDC, .ScaleX(UserControl.Extender.Left, .ScaleMode, vbPixels), .ScaleY(UserControl.Extender.Top, .ScaleMode, vbPixels), vbSrcCopy
    End With
    Exit Sub
ErrorHandler:
End Sub
