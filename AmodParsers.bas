...
Public Function AiparseCreateShapedRegion(cHost As Ac32bppDIB, regionStyle As AeRegionStyles) As Long
    ...
    '*******************************************************
    ' Note: See Ac32bppDIB.CreateRegion for description of the regionStyle parameter
    ...
    If cHost.Alpha = False Then
        AiparseCreateShapedRegion = CreateRectRgn(0&, 0&, cHost.Width, cHost.Height)
        Exit Function
    End If
    ...
    If regionStyle = AregionShaped Then
    ...
    ElseIf regionStyle = AregionEnclosed Then
    ...
    ElseIf regionStyle = AregionBounds Then
...
CleanUp:
    ...
    If Err Then ' failure; probably low on resources
        ...
        Err.Clear
    Else
        AiparseCreateShapedRegion = rtnRegion
    End If
...
Public Function AiparseIsArrayEmpty(FarPointer As Long) As Long
  ' test to see if an array has been initialized
  CopyMemory AiparseIsArrayEmpty, ByVal FarPointer, 4&
End Function

Public Function AiparseByteAlignOnWord(ByVal bitDepth As Byte, ByVal Width As Long) As Long
    ' function to align any bit depth on dWord boundaries
    AiparseByteAlignOnWord = (((Width * bitDepth) + &H1F&) And Not &H1F&) \ &H8&
End Function

Public Function AiparseArrayToPicture(inArray() As Byte, Offset As Long, Size As Long) As IPicture
    ...
    If Not o_hMem = 0& Then
        ...
        If Not o_lpMem = 0& Then
            ...
            If CreateStreamOnHGlobal(o_hMem, 1&, IIStream) = 0& Then
                  Call OleLoadPicture(ByVal ObjPtr(IIStream), 0&, 0&, aGUID(0), AiparseArrayToPicture)
            End If
        End If
    End If
...
Public Function AiparseHandleToStdPicture(ByVal hImage As Long, ByVal imgType As Long) As IPicture
    ...
    aGUID(3) = &HAB0C3000
    ' create stdPicture
    Call OleCreatePictureIndirect(lpPictDesc, aGUID(0), True, AiparseHandleToStdPicture)
    
End Function

Public Function AiparseReverseLong(ByVal inLong As Long) As Long
    ...
    If b1 And &H80& Then
      AiparseReverseLong = ((b1 And &H7F&) * &H1000000 Or &H80000000) Or _
          b2 * &H10000 Or b3 * &H100& Or b4
    Else
      AiparseReverseLong = b1 * &H1000000 Or _
          b2 * &H10000 Or b3 * &H100& Or b4
    End If

End Function

Public Function AiparseValidateDLL(ByVal DllName As String, ByVal dllProc As String) As Boolean
    ...
    If LB Then
        ...
        FreeLibrary LB
    End If
    AiparseValidateDLL = (Not (LB = 0 Or pa = 0))
    
End Function

Public Function AiparseValidateZLIB(ByRef DllName As String, ByRef Version As Long, _
    ...
    ' AcCDECL is one created by Paul Caton that uses assembly to "wrap" the cdecl call into a stdcall.
    ' But we still need to know the calling convention so we know to use AcCDECL or simple API calls.
                ...
                Do
                    ...
                    Select Case asmVal
                        Case &HC3               ' exit code, no stack clean up
                            CopyMemory asmVal, ByVal AiparseSafeOffset(pa, -1&), 1&
                            If Not asmVal = &H33 Then   ' else 0x33C3 is an XOR function, not exit code
                                isCDECL = True      ' DLL uses cdecl calling convention, we use AcCDECL
                                Exit For
                            End If
                        Case &HC2
                            CopyMemory asmVal, ByVal AiparseSafeOffset(pa, 1&), 2&
    ...
    AiparseValidateZLIB = (Not (Version = 0&))
    
End Function


Public Sub AiparseValidateAlphaChannel(inStream() As Byte, bPreMultiply As Boolean, bIsAlpha As Boolean, imgType As Long)
    ...
    '          When routine returns, imgType is either AimgBmpARGB, AimgBmpPARGB or AimgBitmap
...
Public Sub AiparseGrayScaleRatios(Formula As AeGrayScaleFormulas, R As Single, G As Single, B As Single)

        Select Case Formula ' note: when adding your own formulas, ensure they add up to 1.0 or less
        Case AgsclNone   ' no grayscale
            R = 1: G = 1: B = 1
        Case AgsclNTSCPAL
        ...
        Case AgsclBlueGreenMask
            R = 0.1: G = 0.45: B = G    ' personal preferences: could be r=0:g=.5:b=.5 or other weights
        Case AgsclRedGreenMask
            R = 0.45: G = R: B = 0.1    ' personal preferences: could be r=.5:g=.5:b=0 or other weights
        Case Else
            R = 0.299: G = 0.587: B = 0.114 ' use AgsclNTSCPAL
        End Select

End Sub

Public Function AiparseSafeOffset(ByVal Ptr As Long, Offset As Long) As Long
    ...
    Else
        ...
        If Offset < 0 Then ' subtracting from pointer
            If Ptr < MAXINT_4NEG - Offset Then
                ...
                AiparseSafeOffset = MAXINT_4 - ((MAXINT_4NEG - Ptr) - Offset - 1)
            ElseIf Ptr > 0 Then ' verify pointer does not wrap around 0 bit
                If Ptr > -Offset Then AiparseSafeOffset = Ptr + Offset
            Else
                AiparseSafeOffset = Ptr + Offset
            End If
        Else    ' Adding to pointer
            If Ptr > MAXINT_4 - Offset Then
                ' wraps around high bit (forward) & changes to Negative from Positive
                AiparseSafeOffset = MAXINT_4NEG + (Offset - (MAXINT_4 - Ptr) - 1)
            ElseIf Ptr < 0 Then ' verify pointer does not wrap around 0 bit
                If Ptr < -Offset Then AiparseSafeOffset = Ptr + Offset
            Else
                AiparseSafeOffset = Ptr + Offset
...
Public Function AiparseGetFileHandle(ByVal FileName As String, bOpen As Boolean, Optional ByVal useUnicode As Boolean = False) As Long
    ...
    If useUnicode Then
        AiparseGetFileHandle = CreateFileW(StrPtr(FileName), Access, Share, ByVal 0&, Disposition, Flags, 0&)
    Else
        AiparseGetFileHandle = CreateFile(FileName, Access, Share, ByVal 0&, Disposition, Flags, 0&)
    End If

End Function

Public Function AiparseDeleteFile(FileName As String, Optional ByVal useUnicode As Boolean = False) As Boolean
    ...
    If useUnicode Then
        If Not (SetFileAttributesW(StrPtr(FileName), FILE_ATTRIBUTE_NORMAL) = 0&) Then
            AiparseDeleteFile = Not (DeleteFileW(StrPtr(FileName)) = 0&)
        End If
    Else
        If Not (SetFileAttributes(FileName, FILE_ATTRIBUTE_NORMAL) = 0&) Then
            AiparseDeleteFile = Not (DeleteFile(FileName) = 0&)
        End If
    End If

End Function

Public Function AiparseFileExists(FileName As String, Optional ByVal useUnicode As Boolean) As Boolean
    ' test to see if a file exists
    Const INVALID_HANDLE_VALUE = -1&
    If useUnicode = False Then useUnicode = (Not (IsWindowUnicode(GetDesktopWindow) = 0&))
    If useUnicode Then
        AiparseFileExists = Not (GetFileAttributesW(StrPtr(FileName)) = INVALID_HANDLE_VALUE)
    Else
        AiparseFileExists = Not (GetFileAttributes(FileName) = INVALID_HANDLE_VALUE)
    End If
End Function
